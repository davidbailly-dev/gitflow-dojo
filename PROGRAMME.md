...
- **Jour 8** — 2026-08-16 : branche `release/1.0.0` créée depuis `develop`, une correction mineure appliquée, mergée en `--no-ff` dans `main` **et** `develop`, tag `v1.0.0` posé sur `main`. `git log --graph --all --decorate` confirme le tag sur `main` et la correction bien répercutée sur `develop`.
- **Jour 9** — 2026-08-16 : bug critique simulé en prod, branche `hotfix/1.0.1` créée depuis `main` (pas `develop`), correctif appliqué, mergé en `--no-ff` dans `main` **et** `develop`, tag `v1.0.1` posé sur `main`. Rappel fait sur le fait que `git push` ne pousse pas les tags automatiquement (`git push origin <tag>` nécessaire). `git log --graph --all --decorate` confirme les tags `v1.0.0`/`v1.0.1` sur `main` et le hotfix présent des deux côtés.
- **Jour 10** — 2026-08-16 : deux features (`feature/edit-a`, `feature/edit-b`) modifiant la même ligne du README depuis `develop`, conflit croisé provoqué au merge de la seconde (la première passe sans conflit), résolu à la main. Bonus : `feature/edit-c` rebasée sur `develop` avant merge, comparaison faite entre un merge `--no-ff` (garde une trace visible de la feature) et un merge après rebase (fast-forward, historique linéaire mais trace de la branche perdue).
- **Jour 11** — 2026-08-22 : branche `main` protégée sur GitHub (PR obligatoire + 1 review requise), blocage de merge constaté puis contourné en admin ("bypass rules"). Issue `#2` créée et PR `docs/contact-section` rédigée avec une structure Contexte/Changements/Comment tester + `Closes #2`. Remarque importante : la fermeture auto d'issue via `Closes #X` ne fonctionne que si la PR cible la **branche par défaut** du repo (`develop` ici, pas `main`) — l'issue est restée ouverte et a été fermée manuellement. Changement ensuite répercuté de `main` vers `develop` via merge `--no-ff` (conflit mineur résolu à la main).
- **Jour 12** — 2026-08-22 : installation de `git-flow` (AVH edition) via le script officiel, puis `git flow init` avec les conventions déjà en place (`main`/`develop`, préfixes `feature/`, `release/`, `hotfix/`, tag prefix `v`). Mini-cycle rejoué avec l'outil : `feature/search` → `develop` (v1.1.0 en préparation) → `hotfix/1.1.1`. Comparaison avec le travail manuel des Jours 6-9 : l'outil automatise la création + bascule de branche (bonne base à chaque fois), le merge, la suppression de la branche locale (distante seulement si la branche avait été poussée), et la création du tag. Point important découvert en pratique : le comportement `--no-ff` par défaut n'est **pas uniforme** — `feature finish` fait un fast-forward silencieux (pas de commit de merge), alors que `release finish` et `hotfix finish` font bien un vrai merge `--no-ff`.
- **Jour 13-14 (Phase 5, défi final)** — 2026-08-23 : scénario complet enchaîné sans étapes détaillées. `feature/dashboard` et `feature/export` créées et mergées dans `develop` (la première en fast-forward silencieux malgré `gitflow.feature.no-ff true` configuré — la config n'a pas été respectée par l'outil, contournée avec le flag `--no-ff` explicite sur la seconde). `release/2.0.0` démarrée depuis `develop` sans être terminée. Bug critique simulé en prod sur `v1.1.1` : `hotfix/1.1.2` créé depuis `main`, terminé (merge `main`+`develop`, tag `v1.1.2`). Point clé identifié avant exécution (via questions guidées, pas donné directement) : une branche `release` déjà ouverte ne récupère pas automatiquement les commits ajoutés ensuite sur `develop`/`main`, car elle n'est qu'un pointeur figé au moment de sa création — rattrapée avec `git merge main` dans `release/2.0.0` (merge choisi plutôt que rebase, car branche potentiellement partagée). Release terminée, tag `v2.0.0` sur `main` avec les deux features et le hotfix intégrés, répercuté sur `develop`. Revue de code réalisée : bilan positif sur la gestion du rattrapage release/hotfix, points de vigilance notés sur l'incohérence `--no-ff` de l'outil et l'absence de PR pour le hotfix (tout fait en local + push direct en bypass admin).